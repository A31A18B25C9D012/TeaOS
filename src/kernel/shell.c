...
void shell_putchar(char c, uint8_t color) {
    volatile uint16_t *vga = (volatile uint16_t*)0xB8000;
    ...
    if (shell_col < 80) {
        vga[shell_cursor * 80 + shell_col] = ((uint16_t)color << 8) | (uint8_t)c;
        shell_col++;
    }
    if (shell_col >= 80) {
        shell_col = 0;
...
void shell_clear_output(void) {
    for (int y = 1; y < 23; y++)
        fb_clear_region(0, y, 80, 1);
    shell_cursor = 1;
    shell_col = 0;
}
...
void shell_execute(const char *input_buffer) {
    ...
    if (shell_strcmp(input_buffer, "help") == 0 || shell_strcmp(input_buffer, "help -h") == 0) {
        ...
        shell_println("", COLOR_FG);
        shell_println("TeaOS", t.accent);
        shell_println("Small, Simple, Cozy, & Powerful", t.accent);
        shell_println("", COLOR_FG);
        shell_println("       ((         Balanced Ternary: -1  0  +1", t.border);
        shell_println("        ))        8 Ternary Registers (T0-T7)", t.border);
        shell_println("  _ ___((___      TeaScript Assembly Language", t.border);
        shell_println(" (C|        |     Architecture: x86 (32-bit)", t.border);
        shell_println("   `\\      /      Memory: 512 MB", t.border);
        shell_println("     '----'       Mode: VGA Text 80x25", t.border);
        shell_println("", COLOR_FG);
        shell_println("Type 'help' for a list of commands!", t.accent);
        ...
        shell_println("  Version:      1.0", COLOR_FG);
        ...
        *((uint32_t*)&vendor[8]) = ecx;
        ...
        const char *arg = shell_get_arg(input_buffer, 1);
        if (!arg || shell_strcmp(arg, "-h") == 0) {
            shell_println("Usage: asm [-d] <source.asm> [output.bin]", COLOR_FG);
            shell_println("  -h           Show this help", COLOR_FG);
            shell_println("  -d           Show debug output", COLOR_FG);
            shell_println("  Assembles x86 assembly to machine code", COLOR_FG);
            shell_println("  Regs: eax ebx ecx edx esp ebp esi edi", t.accent);
            shell_println("  Ops:  mov add sub xor and or inc dec cmp", t.accent);
            shell_println("  Mem:  mov reg,[reg+off] mov [reg+off],reg", t.accent);
            shell_println("  Byte: movb reg,[r] movb [r],reg shr shl", t.accent);
            shell_println("  Flow: jmp je jne jl jle jg jge call ret", t.accent);
            shell_println("  Data: db val,val,...  nop hlt int", t.accent);
        } else {
            int aoff = 1;
            if (shell_strcmp(arg, "-d") == 0) {
                asm_debug = 1;
                aoff = 2;
                arg = shell_get_arg(input_buffer, 2);
            }
            if (!arg) {
                shell_println("Usage: asm [-d] <source.asm>", COLOR_ERROR);
            } else {
                char outname[32];
                const char *out_arg = shell_get_arg(input_buffer, aoff + 1);
                if (out_arg) {
                    shell_strcopy(outname, out_arg);
                } else {
                    int i = 0;
        ...
        const char *arg = shell_get_arg(input_buffer, 1);
        if (!arg || shell_strcmp(arg, "-h") == 0) {
            shell_println("Usage: run [-d] <binary>", COLOR_FG);
            shell_println("  -h         Show this help", COLOR_FG);
            shell_println("  -d         Show debug output (hex dump)", COLOR_FG);
            ...
            if (!arg) {
                shell_println("Usage: run [-d] <binary>", COLOR_ERROR);
            } else {
                exec_run(arg);
            }
            asm_debug = 0;
